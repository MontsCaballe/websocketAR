...
log_APIResponse = "logs_api.txt"
log_LecturasReport = "logs_lecturas.txt"
        ...
        if device.get('estatus') == '1':  # Verificar si el estatus es '1'
            try:
                ...
                if device['ip'] and await ping_device(device['ip']):  # Solo intentar conectar si el ping tiene éxito
                    try:
                        ip_dispositivo = device['ip']
                        latencia = medir_latencia_tcp(ip_dispositivo)
                        match latencia:
                                case _ if latencia is None:
                                    return "❌ No se pudo medir latencia. Reintentando..."
                                case _ if latencia < 100:
                                    print(f"✅ Latencia segura ({latencia:.2f} ms). Conectando...")
                                    # Limitar el tiempo de conexión a 0.5 segundos
                                    await asyncio.wait_for(connect_to_device(device), timeout=0.5)                                    
                                case _ if 100 <= latencia < 150:
                                    print(f"⚠️ Latencia moderada ({latencia:.2f} ms). Conectar con precaución.")
                                    # Limitar el tiempo de conexión a 0.5 segundos
                                    await asyncio.wait_for(connect_to_device(device), timeout=0.5)
                                case _ if latencia >= 150:
                                    print(f"🚨 Latencia demasiado alta ({latencia:.2f} ms). Descartando conexión.")
                                case _:
                                    print("⚠️ Valor de latencia inesperado.")                        
...
def medir_latencia_tcp(ip):
    ...
    for puerto in puertos:
        ...
        sock.close()
        if resultado == 0:
            port = puerto  # Devuelve el primer puerto abierto encontrado
            break
...
async def main():
    ...
    if not os.path.exists(log_APIResponse):
        with open(log_APIResponse, "w") as f:
            f.write("=== Inicio del Log ===\n")
    if not os.path.exists(log_LecturasReport):
        with open(log_LecturasReport, "w") as f:
            f.write("=== Inicio del Log ===\n")
